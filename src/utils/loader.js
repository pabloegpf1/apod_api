...
exports.getDay = async function (
  ...
  if (body('img').length !== 0) {
    ...
    if (image_thumbnail_size != undefined) {
      ...
      if (multiple_thumbs) {
        ...
        image_thumbnail = `${api_url}image/?image=${
          base_url + img
      ...
      if (image_thumbnail_size != undefined) {
        ...
        if (multiple_thumbs) {
          (async function () {
            ...
            for (let thumb_size of image_thumbnail_size) {
              await array.push(
                `${api_url}image/?image=${await getThumbs(
                  src
                )}&width=${thumb_size}`
              );
          ...
          img_thumb = `${api_url}image/?image=${await getThumbs(
            src
          )}&width=${image_thumbnail_size}`;
        }
  ...
  if (
    dates.getDate(date) < dates.getDate('1996-10-09') &&
    dates.getDate(date) > dates.getDate('1995-09-21')
  ) {
    // it's an APOD structured the old way

    copyright = body('center')
      .eq(0)
      .contents()
      .text()
      .replace(/\n/gm, ' ')
    ...
    description = description
      ...
      .replace(/ {2,}/g, ' ')
      .replace(/^.+Explanation:/, '')
      .replace(/Tomorrow('s|&apos;s) picture:.+/, '')
      .trim();

    title = body('b').eq(0).text().trim().replace(/\n.+/gm, '');
  } else if (
    dates.getDate(date) <= dates.getDate('1995-09-21') &&
    dates.getDate(date) >= dates.getDate('1995-06-16')
  ) {
    // it's an APOD structured the oldest way
    ...
    if (date !== '1995-06-16') {
      copyright = body('body')
        ...
        .replace(/\n/gm, ' ')
        .replace(/ {2,}/g, ' ')
        .match(new RegExp(/Credit:(.*)Explanation/gm))[0]
        .replace('Credit:', '')
        .replace('Explanation', '')
        .trim();
    }
    ...
    // it's an APOD structured the new way
    ...
    copyright = copyright
      ...
      .trim()
      .replace(
        /(?:^Image Credit & Copyright:|^Copyright:|^Credit:|^Credit and copyright:|^Image Credit:)/gim,
        ''
      );
